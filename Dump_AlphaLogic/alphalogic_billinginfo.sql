...
CREATE TABLE `billinginfo` (
  ...
  `billDate` timestamp(4) NULL DEFAULT NULL,
  `payId` int(255) DEFAULT NULL,
  `oId` int(255) DEFAULT NULL,
  `bTransactionId` varchar(255) DEFAULT NULL,
  `bTransactionStatus` varchar(45) DEFAULT NULL,
  PRIMARY KEY (`billingId`),
  KEY `payment-billing_idx` (`payId`),
  KEY `order-billing_idx` (`oId`),
  CONSTRAINT `order-billing` FOREIGN KEY (`oId`) REFERENCES `order` (`oId`) ON DELETE CASCADE ON UPDATE NO ACTION,
  CONSTRAINT `payment-billing` FOREIGN KEY (`payId`) REFERENCES `payment` (`payId`) ON DELETE CASCADE ON UPDATE NO ACTION
